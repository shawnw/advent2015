...
// Stolen shamelessly from http://stackoverflow.com/a/25497877
...
int main(int argc, char **argv) {
	...
	//std::sort(jugs.begin(), jugs.end());
	std::vector<int> dp(151, 0);
	dp[0] = 1;
	for (auto cur_num : jugs) {
		for (n = 150; n >= cur_num; n--)
			dp[n] += dp[n - cur_num];
	}
	std::cout << dp[150] << '\n';
	return 0;
